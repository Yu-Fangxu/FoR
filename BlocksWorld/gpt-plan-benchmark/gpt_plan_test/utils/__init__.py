...
def instance_to_text_blocksworld(problem, get_plan, plan, data, shuffle=False):
    """
    ...
    if get_plan:
        ...
        plan = plan.replace("yellow", "yellow-block").replace("red", "red-block").replace("blue", "blue-block").replace("orange", "orange-block")
        plan = plan.split("\n")[:-1]
        # print(plan)
        for action in plan:
